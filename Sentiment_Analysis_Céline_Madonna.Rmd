...
Looking at the individual datasets, shows us Céline Dion uses the word "love" the most often, than Madonna.

```{r}
celine_rank<- celine_df %>%
  count(word, sort = TRUE) 
celine_rank
```

```{r}
madonna_rank <- madonna_df %>% 
  count(word, sort = TRUE)
madonna_rank
```
...
Let's count the most common positive and negative words using the bing lexicon.
...
celine_bing %>% 
  ...
  coord_flip() +
  labs(y = "Sentiment Score") +
  ggtitle("Sentiment for Céline Dion")
```
...
Céline Dion is all about love :) But in general, from a bird's eye view, her words, are really a lot more romantic, emotional and positive.
...
Below I will create a grpah that will explicitly show which artist is more positive by the "nrc" lexicon:
...
I will be creating 4 topics, and see what type of words I get in each.
...
https://github.com/simranvatsa/tayloR/blob/master/tayloR_final.R

http://www.sthda.com/english/wiki/text-mining-and-word-cloud-fundamentals-in-r-5-simple-steps-you-should-know

https://github.com/thegrigorian/TextAnalysisWithGutenberg

https://github.com/BenceToth/nlp_songlyrics/blob/master/DS4_final-assignment_Bence-L-Toth_166504.rmd
